...
export function jdiSegment({ cwd, home, cache = {}, rotateMs = 30000, cmpVer, colors, now = Date.now() }) {
  ...
  const fx = EFFECTS[Math.floor(now / 5000) % EFFECTS.length]; // estilo muda a cada ~5s
  const decorated = fx(ad, Math.floor(now / 1000));            // 1 quadro/seg — anima com refreshInterval:1
