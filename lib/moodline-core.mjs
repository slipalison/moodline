...
export const DEFAULT_CFG = {
  ...
  features: {
    ...
    coauthor: false,         // 🤝 quando os commits levam Co-Authored-By — opt-in (evita segmento novo forcado)
  },
};
...
// ---------------- co-autoria de commit ----------------
// Le a config da PROPRIA CLI (nao o JSON do host) pra saber se os commits levam Co-Authored-By.
// Claude Code: `attribution.commit === ''` desliga (Generated + Co-Authored-By); a legada
// `includeCoAuthoredBy:false` idem; ausente = default LIGADO. Copilot CLI: sempre liga (hardcoded,
// nao configuravel). Outras CLIs (gemini/opencode/junie): sem o conceito -> null (sem indicador).
export function coauthorState(adapter, home = homedir()) {
  if (adapter === 'copilot') return true;
  if (adapter !== 'claude') return null;
  try {
    const s = JSON.parse(readFileSync(join(home, '.claude', 'settings.json'), 'utf8'));
    if (s.attribution && typeof s.attribution.commit === 'string') return s.attribution.commit.trim() !== '';
    if (s.includeCoAuthoredBy === false) return false;
    return true; // default do Claude Code: ligado
  } catch {
    return true; // sem settings.json / ilegivel: o default do Claude e ligado
  }
}
...
export function render(state, cfg = DEFAULT_CFG) {
  ...
  const opt = [
    ...
    f.coauthor && state.coauthor && { txt: `${DIM}\u{1F91D}${RESET}`, prio: 5, line: 1 }, // 🤝 discreto quando o co-autor esta ligado
...
export function attachGit(state, useGit) {
  if (state.gitBranch) {
    // branch vinda do host (Copilot/Antigravity): zero spawn; dirty so se o host informar
    state.git = { branch: state.gitBranch, dirty: !!state.gitDirty, ahead: 0, behind: 0 };
  } else if (useGit) {
...
// Antigravity CLI (`agy`, sucessor do Gemini CLI) — statusLine por comando, payload proprio:
// context_window espelha o Claude; quota{gemini-5h,gemini-weekly}.remaining_fraction (0..1) vira
// rate usado%; vcs{branch,dirty} vem do host (zero spawn de git); sem custo/effort no schema.
export function fromAntigravity(j) {
  const cw = j.context_window || {};
  const q = j.quota || {};
  const used = (b) => (typeof b?.remaining_fraction === 'number' ? (1 - b.remaining_fraction) * 100 : null);
  const five = used(q['gemini-5h']);
  const seven = used(q['gemini-weekly']);
  return {
    ...
    linesAdded: null,
    linesRemoved: null,
    rate: five != null || seven != null ? { five, seven } : null,
    cwd: j.cwd || null,
    gitBranch: j.vcs?.branch || null,
    gitDirty: !!j.vcs?.dirty,
    repo: null,
  };
}
...
// Auto-configuravel: sem --config, usa o config.json AO LADO do engine; sem --adapter, usa o
// `adapter` gravado no config (o Antigravity CLI nao interpreta aspas/args no comando — o
// settings dele aponta so `node <core.mjs>`, sem argumentos).
export function buildLine(rawText, args = {}) {
  const cfg = loadConfig(args.config || join(SELF_DIR, 'config.json'));
  const adapterName = String(args.adapter || cfg.adapter || 'claude').toLowerCase();
  ...
  try {
    ...
    state.coauthor = cfg.features.coauthor ? coauthorState(adapterName) : null;
