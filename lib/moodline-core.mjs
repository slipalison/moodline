...
import { jdiSegment, globalJdiVersion, fetchJdiLatest } from './jdi.mjs';
...
// Cache curto por cwd (TTL 3s): com refreshInterval:1 nao faz sentido spawnar git todo segundo.
function gitInfo(cwd) {
  if (!cwd || !existsSync(cwd)) return null;
  const key = [...cwd].reduce((h, c) => (h * 31 + c.charCodeAt(0)) >>> 0, 7).toString(36);
  const cacheFile = join(tmpdir(), `moodline-git-${key}.json`);
  try { const c = JSON.parse(readFileSync(cacheFile, 'utf8')); if (Date.now() - c.at < 3000) return c.data; } catch {}
  const data = computeGitInfo(cwd);
  try { writeFileSync(cacheFile, JSON.stringify({ at: Date.now(), data })); } catch {}
  return data;
}
...
export function updateBadge(cfg) {
  if (!cfg.version) return null;
  const c = readUpdateCache();
  return c && c.latest && cmpVer(c.latest, cfg.version) > 0 ? c.latest : null;
}
export function maybeSpawnCheck() {
  const c = readUpdateCache();
  if (c && c.checkedAt && Date.now() - c.checkedAt < DAY_MS) return; // ainda fresco
  try {
    // carimba ja pra evitar enxame de spawns durante o debounce de 300ms
    writeFileSync(cachePath(), JSON.stringify({ ...(c || {}), checkedAt: Date.now() }));
...
export function render(state, cfg = DEFAULT_CFG) {
  ...
  core += ` ${mood(state.pct)} ${c}${state.pct}%${RESET} ${DIM}${tokLabel(state.tokens, state.ctxSize)}${RESET}`;

  // segmentos opcionais. prio = ordem de queda quando estreito (single). line = linha no layout multi:
  // linha 1 (com o nucleo) = metricas (update/custo/rate); linha 2 = contexto + diversao (git/JDI/puns).
  const opt = [];

  // badge de atualizacao disponivel (nudge discreto, alta prioridade)
  if (state.update) opt.push({ txt: `${MAGENTA}⬆ v${state.update}${RESET}`, prio: 1, line: 1 });

  // JDI: anuncio ocupa o slot do trocadilho (prio 4); aviso de update fica em destaque (prio 1). Linha 2.
  if (state.jdi) opt.push({ txt: state.jdi.txt, prio: state.jdi.ad ? 4 : 1, line: 2 });
  ...
  if (f.cost) {
    ...
    if (parts.length) opt.push({ txt: parts.join(' '), prio: 2, line: 1 });
  }
  ...
  if (f.rate && state.rate && (state.rate.five != null || state.rate.seven != null)) {
    ...
    opt.push({ txt: `⏳ ${parts.join(' ')}`, prio: 3, line: 1 });
  }

  // trocadilho — suprimido quando o anuncio do JDI esta ocupando o lugar dele
  if (f.puns && !(state.jdi && state.jdi.ad)) {
    opt.push({ txt: `${DIM}\u{1F4AC} ${pickPun(cfg.punRotateMs ?? 30000, cfg.extraPuns)}${RESET}`, prio: 4, line: 2 });
  }
  ...
  if (cfg.layout === 'multi') {
    ...
    const l1 = opt.filter((s) => s.line === 1).sort(byPrio).map((s) => s.txt); // metricas, junto do nucleo
    const l2 = opt.filter((s) => s.line !== 1).sort(byPrio).map((s) => s.txt); // git + JDI + trocadilho
...
export function buildLine(rawText, args = {}) {
  ...
  try {
    ...
    state.update = updateBadge(cfg);   // le cache (sync, rapido)
    state.jdi = jdiSegment({ cwd: state.cwd, cache: readUpdateCache() || {}, rotateMs: cfg.punRotateMs ?? 30000, cmpVer, colors: { MAGENTA, CYAN, DIM, RESET } });
    maybeSpawnCheck();                 // dispara busca em background no maximo 1x/dia
...
// Entry point IO: le stdin, escreve stdout.
export function runMain(argv) {
  const args = parseArgs(argv);
  if (args['update-check']) { doUpdateCheck(); return; } // modo do processo filho destacado
  process.stdout.write(buildLine(readStdin(), args) + '\n');
}
