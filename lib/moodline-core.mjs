...
export function render(state, cfg = DEFAULT_CFG) {
  ...
  if (f.git) { const g = gitSegment(state); if (g) opt.push(g); }
  if (f.cost) { const co = costSegment(state); if (co) opt.push(co); }
  if (f.rate) { const ra = rateSegment(state); if (ra) opt.push(ra); }
  if (f.puns && !state.jdi?.ad) opt.push(punSegment(cfg)); // anuncio do JDI ocupa o slot do pun
