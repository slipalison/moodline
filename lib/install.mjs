...
description: Liga/desliga segmentos da statusline moodline (git, cost, rate, puns), tamanho da barra e layout — sem sair do Claude Code
allowed-tools: Bash(moodline:*), Bash(npx:*)
---
...
Aplique a mudança rodando \`moodline config --cli=claude\` com as flags certas:
- ligar/desligar feature: \`--toggle git|cost|rate|puns\` (ou \`--on git,cost\` / \`--off puns\`)
- tamanho da barra: \`--bar=10\`
- layout: \`--layout=single\` ou \`--layout=multi\`
- atualizar o moodline (se houver versão nova): \`moodline update\`

Se o pedido estiver vazio, apenas mostre o config atual e liste as opções. Rode um único comando e confirme em uma linha — a barra atualiza no próximo refresh (sem reiniciar).
`;
