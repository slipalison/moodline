...
export function render(state, cfg = DEFAULT_CFG) {
  ...
  if (state.update) opt.push({ txt: `${MAGENTA}⬆ v${state.update}${RESET}`, prio: 1 });

  // JDI: anuncio ocupa o slot do trocadilho (prio 4); aviso de update fica em destaque (prio 1).
  if (state.jdi) opt.push({ txt: state.jdi.txt, prio: state.jdi.ad ? 4 : 1 });
