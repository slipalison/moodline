...
export function targets(home = homedir()) {
  ...
  return {
    ...
    // Antigravity CLI (`agy`): config herdada do Gemini CLI em ~/.gemini/antigravity-cli/
    antigravity: mk('antigravity', 'Antigravity CLI', join('.gemini', 'antigravity-cli'), 'antigravity'),
  };
}
...
function commandFor(t) {
  // Antigravity CLI executa o comando SEM shell (split ingenuo por espaco, aspas viram literais)
  // -> comando bare, sem aspas nem args; o engine le adapter/config do config.json ao lado dele.
  // Limitacao do agy: caminho de HOME com espaco quebra o split deles (nao ha como escapar).
  if (t.adapter === 'antigravity') return `node ${fwd(t.core)}`;
...
function setStatusLine(s, t) {
  ...
  } else if (t.adapter === 'antigravity') {
    // schema do agy (exemplo oficial + issue #320): `enabled` liga; sem padding/refreshInterval
    s.statusLine = { type: 'command', command, enabled: true };
  } else {
...
export function configure(key, { features, layout, home } = {}) {
  ...
  cfg.adapter = t.adapter;    // engine auto-configuravel: le o adapter daqui quando roda sem args
