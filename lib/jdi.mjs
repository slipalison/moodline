...
const NPM_URL = 'https://www.npmjs.com/package/jdi-cli';
const REGISTRY = 'https://registry.npmjs.org/jdi-cli/latest';

// hyperlink OSC 8 (terminais que suportam viram clicavel; os outros mostram so o texto)
const link = (label, url) => `\x1b]8;;${url}\x1b\\${label}\x1b]8;;\x1b\\`;
...
// JDI no projeto local: instalado em node_modules (com versao) ou so declarado no package.json.
export function localJdi(cwd) {
  if (!cwd) return { installed: false, version: null };
  try {
    const pj = join(cwd, 'node_modules', 'jdi-cli', 'package.json');
    if (existsSync(pj)) return { installed: true, version: JSON.parse(readFileSync(pj, 'utf8')).version || null };
  } catch {}
  try {
    const proj = join(cwd, 'package.json');
    if (existsSync(proj)) {
...
export function jdiSegment({ cwd, cache = {}, rotateMs = 30000, cmpVer, colors, now = Date.now() }) {
  ...
  if (isInstalled) {
    ...
    return hasUpdate
      ? { txt: `${MAGENTA}⬆ JDI v${cache.jdiLatest}${RESET} ${DIM}${link('[atualizar]', NPM_URL)}${RESET}`, ad: false }
  ...
  return { txt: `${DIM}✨ ${ad}${RESET} ${CYAN}${link('[veja aqui]', NPM_URL)}${RESET}`, ad: true };
}
