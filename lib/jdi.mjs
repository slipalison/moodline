...
// Versao da release do JDI gravada no projeto, se houver (.jdi/config.json). Hoje o JDI grava
// "$schema_version" (schema de estado, nao a release) — entao isto so retorna algo quando o JDI
// passar a gravar "jdi_version"/"version". Permite o aviso de update mesmo rodando via npx.
export function jdiProjectVersion(cwd) {
  if (!cwd) return null;
  let dir = cwd;
  for (;;) {
    try {
      const cfg = join(dir, '.jdi', 'config.json');
      if (existsSync(cfg)) {
        const j = JSON.parse(readFileSync(cfg, 'utf8'));
        return j.jdi_version || j.jdiVersion || j.version || null;
      }
    } catch {}
    const parent = dirname(dir);
...
export function jdiSegment({ cwd, home, cache = {}, rotateMs = 30000, cmpVer, colors, now = Date.now() }) {
  ...
  if (present) {
    const v = jdiProjectVersion(cwd) || cache.jdiGlobal; // versao no projeto (npx) ou instalador global
