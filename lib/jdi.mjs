...
import { safeDir } from './pathguard.mjs';
...
// `cwd` (entrada externa) e validado por safeDir antes de qualquer acesso ao FS (S8707).
export function jdiInProject(cwd) {
  let dir = safeDir(cwd);
  if (!dir) return false;
  for (;;) {
    // NOSONAR(S8707): `dir` validado por safeDir; e o diretorio de trabalho do usuario, sem confinamento a escapar.
    try { if (statSync(join(dir, '.jdi')).isDirectory()) return true; } catch {} // NOSONAR
...
// Le a versao gravada numa pasta .jdi/: VERSION (prioridade) ou jdi_version/version no config.json.
// `jdiDir` validado por safeDir (diretorio real) antes das leituras (S8707).
function readJdiVersion(jdiDir) {
  const dir = safeDir(jdiDir);
  if (!dir) return null;
  try {
    const vfile = join(dir, 'VERSION');
    // NOSONAR(S8707): `dir` validado por safeDir; leitura no diretorio do proprio usuario.
    if (existsSync(vfile)) { const v = readFileSync(vfile, 'utf8').trim(); if (v) return v; } // NOSONAR
  } catch {}
  try {
    const cfg = join(dir, 'config.json');
    if (existsSync(cfg)) {
      const j = JSON.parse(readFileSync(cfg, 'utf8')); // NOSONAR(S8707): dir validado por safeDir
...
export function jdiProjectVersion(cwd) {
  ...
  for (;;) {
    ...
    if (existsSync(jdiDir)) return readJdiVersion(jdiDir);
    const parent = dirname(dir);
...
export function jdiSegment({ cwd, home, present, projectVersion, cache = {}, rotateMs = 30000, cmpVer, colors, now = Date.now() }) {
  const { MAGENTA = '', CYAN = '', RESET = '' } = colors || {};
  if (present === undefined) present = jdiInProject(cwd) || jdiInRuntime(home);

  if (present) {
    const v = (projectVersion === undefined ? jdiProjectVersion(cwd) : projectVersion) || cache.jdiGlobal; // projeto (npx) ou instalador global
