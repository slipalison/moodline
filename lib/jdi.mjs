...
function fxScan(t, frame) {
  const cs = [...t];
  const pos = cs.length ? frame % cs.length : 0;
  let out = '';
...
export function jdiSegment({ cwd, home, cache = {}, rotateMs = 30000, cmpVer, colors, now = Date.now() }) {
  ...
  const fx = EFFECTS[Math.floor(now / 6000) % EFFECTS.length]; // troca de estilo ~6s (nao repetitivo)
  const decorated = fx(ad, Math.floor(now / 700));             // frame da pseudo-animacao
