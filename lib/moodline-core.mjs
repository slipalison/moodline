...
function ttlCache(ns, key, ttlMs, compute) {
  const h = [...key].reduce((a, c) => (a * 31 + c.charCodeAt(0)) >>> 0, 7).toString(36);
...
// git: TTL 3s (muda com frequencia, mas nao a cada segundo).
function gitInfo(cwd) {
  if (!cwd || !existsSync(cwd)) return null;
  return ttlCache('git', cwd, 3000, () => computeGitInfo(cwd));
}
...
// (event loop congelado), o processo nao sai e acumula ("Suspended"). Aqui o watchdog resolve a
// promessa mesmo sem EOF. `stdin`/`timeoutMs` sao injetaveis pra teste (DIP). Watchdog unref'd:
// nunca segura o processo vivo sozinho.
...
export function buildLine(rawText, args = {}) {
  ...
  try {
    ...
    const jdi = jdiInfo(state.cwd);    // deteccao cacheada (TTL 60s) — sem varrer FS todo refresh
...
if (isMain) runMain(process.argv.slice(2)).catch(() => process.exit(1)); // nunca deixa promessa pendurar
