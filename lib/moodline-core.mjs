...
// ---------------- git ----------------
// So a branch, no timeout curto — fallback pra repo gigante onde o status estoura o tempo.
function gitBranchOnly(cwd) {
  try {
...
export function computeGitInfo(cwd) {
  const opt = { cwd, encoding: 'utf8', timeout: 500, stdio: ['ignore', 'pipe', 'ignore'] };
  try {
    const out = execFileSync('git', ['status', '--porcelain=v2', '--branch'], opt);
    ...
    return gitBranchOnly(cwd); // status estourou (repo gigante)? mostra ao menos a branch
  }
}
